...
echo sudo podman build --build-arg OS_RELEASE=${OS_RELEASE} --build-arg OS_IMAGE=${OS_IMAGE} --build-arg DEVBUILD=${DEVBUILD} -t ${BUILD_ARCH}/${IMAGE_NAME}:${BUILD_ID} -f contrib/podman-fedora/Containerfile
